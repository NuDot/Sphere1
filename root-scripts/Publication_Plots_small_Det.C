...
void Publication_Plots_small_Det(){
  ...
  std::string simulation = "/home/christoph/geant4/MyG4_applications/Sphere1-build/output/20May2013_smallDet_redQE";
  ...
  TH1F * time_afterTTS_Cerenkov = new TH1F("time_afterTTS_Cerenkov",";Time [ns]; PEs per event/0.1 ns",5000,0.,50.);   
  TH1F * time_afterTTS_Cerenkov_full = new TH1F("time_afterTTS_Cerenkov_full",";Time [ns]; PEs per event/0.1 ns",10000,0.,1000.);  
  TH1F * time_afterTTS_Scintillation = new TH1F("time_afterTTS_Scintillation",";Time [ns]; PEs per event/0.1 ns",5000,0.,50.);
  ...
  TH1F * cos_psi_3p56_h = new TH1F("cos_psi_3p56_h",";cos(#Psi);PEs per event/0.02",100,-1.,1.);
  ...
  Hits_Tree->Draw("x_hit/sqrt(x_hit*x_hit + y_hit*y_hit + z_hit*z_hit) >> cos_psi_3p56_h","PE_creation == 1 && PE_time < 3.56");
  ...
  cos_psi_3p56_h->Scale(1./N_events_double);
  ...
  int count2=0;
  int count2_norm=0;
  count1=time_afterTTS_Cerenkov->Integral(  time_afterTTS_Cerenkov->FindBin(0.)  ,  time_afterTTS_Cerenkov->FindBin(3.56) -1 );  
  ...
  count2=time_afterTTS_Scintillation->Integral(  time_afterTTS_Scintillation->FindBin(0.)  ,  time_afterTTS_Scintillation->FindBin(3.56) -1 );
  ...
  cout << "Cerenkov integral [0.,3.56] = " << count1 << ", Cerenkov integral [0.,1000.] = " << count1_norm << ", Cerenkov integral [0.,34.] = " << count3 << endl; 

  cout << "Scintillation integral [0.,3.56] = " << count2 << ", Scintillation integral [0.,1000.] = " << count2_norm << ", Scintillation integral [0.,34.] = " << count4 << endl;


  TNtuple *tn_count = new TNtuple("tn_count","","int_cer_3p56:int_cer_34:int_cer_all:int_scint_3p56:int_scint_34:int_scint_all");
  ...
  //plot2
  TCanvas* c2 = new TCanvas("c2", "Cos Psi 3.56 ns time cut", width_pixels, height_pixels); 
  cos_psi_3p56_h->Draw();
  jStyle->SetFrameLineWidth(3); 
  //theta_phi_1event->SetMarkerColor(2);
  //theta_phi_1event->SetMarkerStyle(20);
  cos_psi_3p56_h->GetXaxis() -> CenterTitle(true);
  cos_psi_3p56_h->GetYaxis() -> CenterTitle(true);
  cos_psi_3p56_h->SetMinimum(0.);
  //cos_psi_33_h->GetXaxis()->SetTitleSize(0.08);
  //cos_psi_33_h->GetYaxis()->SetTitleSize(0.08);
  c2->Update();
  c2->SaveAs(Form("%s/cos_psi_3p56_h.png",out.c_str()),"png");
  c2->SaveAs(Form("%s/cos_psi_3p56_h.C",out.c_str()),"C");
  c2->SaveAs(Form("%s/cos_psi_3p56_h.pdf",out.c_str()),"pdf");
  ...
  TCanvas* c4 = new TCanvas("c4", "Cos Psi 3.56 and 34.0 ns time cut comparison", width_pixels, height_pixels);
  //cos_psi_34_h->SetLineColor(2);
  //cos_psi_34_h->SetFillColor(2);
  //cos_psi_34_h->SetFillStyle(3004);
  cos_psi_34_h->Draw();
  cos_psi_34_h->SetMinimum(0.);
  cos_psi_3p56_h->Draw("same");
  jStyle->SetFrameLineWidth(3);
  cos_psi_34_h->GetXaxis() -> CenterTitle(true);
  cos_psi_34_h->GetYaxis() -> CenterTitle(true);
  c4->Update();
